...
const firebase = {
  ...
  async create_references(files, type) {
    ...
    try {
      ...
      for (const key in files) {
        ...
        if (key != 'item' && key != 'length') references[i] = ref(storage, pass.email + '/' + type + '/' + files[key].name);//não conta com nomes de atributos ou metodos do FileList.. (item/length)
